...
- 检查时间（UTC）：2026-09-22T07:12:00.737760Z　代码版本：7114319
...
| 二·数据格式 | JSON 顶层必须写 platform 与 commit | 已整改 | 向 387 个结果 JSON 注入 platform/commit/run_id/scenario/produced_at（脚本幂等回填 + 驱动层持久化）。 |
...
| 三·必填记录 | 实验台账 + README 记录 §三 全部字段 | 已整改 | 生成 docs/实验台账.md + results/experiment_ledger.json（377 个 run）；README 补合规附录。 |
...
| 三·必填记录 | 同场景重复≥10次 + 均值/波动/CI（正式对比） | **基本符合（主场景已达 ≥10；其余待补）** | 归档 run 共 377 个（台账口径），单场景 run 数 >10，但多为**同种子不同参数**的扫描点/双轨点；**独立种子目前仅 2–3 个**（主 20260901、crossval 42、`--reps 3` 追加 7）。正式对比已用 **3 种子 + 95% CI**（README §2.5）；**2026-09-23 已对主场景 storm2 补跑 10 独立种子 + 95% CI**（产物 `results/多种子统计_storm2_10seed_20260923.json`；如切换总时延均值 12.063 ms、95% CI [12.054, 12.072]）；**其余场景仍为 1–3 种子**，未遍及。该批**同时覆盖 Rel-17 受控对照**的 10 种子版本（Rel-17 四步基线 380.91 ms ↔ 本方案 12.06 ms），故「本方案 ↔ Rel-17」这一**正式对比亦达 ≥10 次独立重复**。 |

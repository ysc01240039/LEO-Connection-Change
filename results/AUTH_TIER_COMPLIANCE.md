...
> 生成时间：2026-09-16T12:55:20.795623Z
...
| 指标 | 规范口径 | 项目现状 | 符合 |
|---|---|---|---|
| 伪造终端拦截率 | 成功拦截数 / 伪造总数 | `伪造终端拦截率`(=block_rate)，实测 0.8322 ± 0.0728 (n=307) | ✅ |
| 认证时延 | 认证开始→结果返回 | `认证引入额外时延_ms`(=auth_latency_ms)，实测 0.1048 ± 0.0147 (n=307) | ⚠️ 口径为「认证引入额外时延」(HMAC 校验×降频)，非完整认证往返；起止事件按规范备注待 A 组结合代码统一 |

...
def rel17_improvement(base: dict, prop: dict) -> dict:
    ...
    **设计取舍而非缺陷**。现改为：
      (1) 主 KPI 列表以正向量打头：接入时延↓、高危/中危/低危成功率↑、切换中断↓；
      (2) 聚合成功率单列并标注「吞吐-公平性权衡（设计取舍）」，不再作为方案成败判据。
    ★注（2026-09-23）★：本函数**不产出**「生存优先效用提升%」——分层加权效用由
    `_prio_utility()` 单独计算（供诊断脚本 `perf/check_priority.py` 与报告卡使用），未纳入下方 spec；
    原 docstring 曾声称已纳入、与实现不符，现更正（纯注释，不改任何计算）。
    """
    spec = [
        ("接入时延均值_ms", "降低"),
        ("high危终端接入成功率", "提升"),
